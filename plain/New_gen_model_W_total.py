...
num_list_x_start_10k_2, num_list_x_end_10k_2 = split_data(data_df, window_size)
...
data_X_train = read_data("data_mod/target_training_mod.txt")
data_X_test = read_data("data_mod/target_testing_mod.txt")
data_Y_1 = [data_X_train, data_X_test]
data_Y_train = read_data("data_mod/tag_training_mod.txt")
data_Y_test = read_data("data_mod/tag_testing_mod.txt")
...
def DNNmodel(numbering):
    ...
    W.to_csv('../encrypted/Total_DNNmodels/DNNmodels_' + str(window_size) + '/W_New' + str(numbering)  + '.csv', header = False, index =False)
